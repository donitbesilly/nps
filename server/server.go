...
//MaxConnMapDotNum caps how many recent connections are plotted as dots on
//the connection-map page. The full history remains in the connlog database
//regardless and is browsed separately (paginated, never auto-pruned).
const MaxConnMapDotNum = 50

//GetRecentConnLogs returns the most recent connections across every tunnel,
//newest first, for plotting on the connection map.
func GetRecentConnLogs() []*connlog.Record {
	list, err := connlog.Recent(MaxConnMapDotNum)
	if err != nil {
		logs.Warn("failed to read recent connection logs: %s", err.Error())
		return []*connlog.Record{}
	}
	return list
}
